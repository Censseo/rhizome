...
| E2E-64 | Boot a real node from a genesis snapshot already funded at twice the curve's supply target, so the first curve-active block's parent supply sits deep in the curve's negative (mirrored) branch where the honest reward clamps to exactly 0, then claim any positive reward there, hoping the single clamp site is reachable only through a hand-built `NetworkParameters` and not through a real node's actual validation path. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aPositiveRewardClaimedAboveTheSupplyTargetIsRejectedAndTheNodeStaysHealthy` |
| E2E-65 | Mine a real chain into the real, positive-width band immediately below the curve's supply target where the honest reward genuinely floors to exactly 0, then both claim one extra unit there and verify the node keeps producing through several more genuinely-zero-reward blocks, hoping "expected reward is zero" gets silently treated as "no check ran" somewhere in the real path. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#theRealNodeKeepsProducingThroughAGenuineZeroRewardBlockAndRejectsAnyExtraUnitClaimedThere` |
...
| E2E-75 | Mine a real node, with a genuine live block producer, far past the curve's supply target into the sustained region where the honest reward is pinned to exactly 0 for many consecutive real blocks, hoping the sustained zero eventually gets confused with an unrelated mining-fee status, or that the real `/wallet` balance route or some other downstream code silently assumes a non-zero coinbase. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aRealNodeMinedDeepPastTheSupplyTargetProducesManyZeroRewardBlocksAndStaysHealthy` |
