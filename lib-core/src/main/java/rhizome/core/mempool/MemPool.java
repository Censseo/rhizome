...
public final class MemPool {
    ...
    public ExecutionStatus addTransaction(Transaction transaction) {
        // Structural gates first, before the lock: they are pure (no pool state) and mirror the
        // executor's consensus gates, so a rejected transaction costs no lock acquisition and no
        // pool mutation — the pool's cheapest-first DoS doctrine. See TransactionAdmission.
        ExecutionStatus gate = TransactionAdmission.check(params, accounts, clock, transaction);
        if (gate != SUCCESS) {
            return gate;
        }
