...
The miner revenue floor `R_min` (`NetworkParameters.minerRevenueFloor`, 800 base units on mainnet)
replaces feature 03's zero clamp at the curve's single clamp site: the scheduled base reward is
`max(R_min, raw(S))` — strictly positive at every curve-active height and every supply, flat `R_min`
from the crossover `S ≈ 0.9669 × S*` onward. The floor is the defence against the hashrate-collapse
endgame (research.md Decision 3): under a zero subsidy the finality window's rewrite cost collapses
to `maxReorgDepth × 2^minDifficulty` when the chain carries the most value. The family proves the
floor holds as a consensus fact across the whole reachable domain including wire-legal extremes
(FLOOR-01), that a validator enforces the floored schedule exactly before proof-of-work (FLOOR-02),
that uncle/nephew rewards floor through the base with no second clamp (FLOOR-03), and that a
non-positive floor refuses to build (FLOOR-04). The domain sweep and degenerate-config refusal also
have non-adversarial twins in `lib-core/src/test/java/rhizome/NetworkParametersTest.java`
(`minerRevenueNeverFallsBelowTheFloor`, `aNonPositiveMinerRevenueFloorRefusesToBuild`); the attack
suite carries the catalogue-cited copies because `AdversarialProtocolTest` scans only
`rhizome.adversarial.*`.

| ID | Scenario | Class | Verdict | Proof |
|----|----------|-------|---------|-------|
| FLOOR-01 | Push the dispatched reward across the reachable supply domain, including wire-legal extreme supplies near `Long.MAX_VALUE` and the deep ratio-mirror branch, hoping the floor slips anywhere — a reward below `R_min`, a zero, or a negative — or that the extreme supply makes the accounting identity wrap instead of failing loudly. | A3 | DEFENDED | `lib-core/src/test/java/rhizome/adversarial/MinerRevenueFloorAttackTest.java#theDispatchedRewardFloorsAtExactlyRMinAcrossTheReachableDomain` |
...
| E2E-61 | Push a block built by the unmodified `E2EFixtures.build`/`mint` fixture onto a real curve-active node, hoping the curve-sensitive supply stamp and a curve-insensitive coinbase are close enough that the real consensus engine lets the contradiction through. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aCurveAwareFixtureBuiltBlockIsAcceptedOnACurveActiveNodeWhileTheLegacyFixtureBuiltBlockIsRejected` |
| E2E-62 | Mine a block with genuinely valid proof of work under a curve-active profile, keep the header's committed supply honest, but pay the coinbase a neighbouring curve table position's reward instead of this block's own, then push it at the real `/submit` HTTP route, hoping a plausible-looking (schedule-derived, just wrong-position) forgery is harder to catch than an arbitrary one. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aCurveTableNeighbourRewardPushedAtTheSubmitRouteIsRejectedAndAnHonestBlockAtTheSameHeightIsAccepted` |
| E2E-63 | Boot a real node from a genesis snapshot whose committed supply is exactly a generated emission-curve table position — honest mining's consensus-chosen reward granularity cannot land on one — then submit a coinbase exactly one base unit above the true value at that exact parent supply, hoping the `floorDiv` interpolation at a segment boundary, a potentially negative numerator, or the zero-width last segment, lets a one-unit overstatement through. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aOneBaseUnitOverstatedRewardAtATableStepBoundaryIsRejectedAndTheNodeStaysHealthy` |
| E2E-64 | Boot a real node from a genesis snapshot already funded at twice the curve's supply target, so the first curve-active block's parent supply sits deep in the curve's negative (mirrored) branch where the honest reward is the floor, exactly `R_min`, then claim any amount off the floor there (`R_min + 1`, or `0`), hoping the single clamp site is reachable only through a hand-built `NetworkParameters` and not through a real node's actual validation path. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aRewardOffTheFloorClaimedAboveTheSupplyTargetIsRejectedAndTheNodeStaysHealthy` |
| E2E-65 | Mine a real chain through the floored crossover band — the stretch below the curve's supply target where the honest reward is exactly `R_min` — then both claim an extra unit there and verify the node keeps producing through several more floored blocks, hoping the flat `R_min` reward gets silently treated as "no check ran" somewhere in the real path. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#theRealNodeKeepsProducingThroughTheFloorCrossoverBandAndRejectsAnyExtraUnitClaimedThere` |
| E2E-66 | Fill a genuinely-mined block's uncle list with `UncleRef`s naming hashes no orphan pool has ever seen, compute the header's committed supply to be self-consistent with that lie via the same formula `checkSupply` itself uses, keep the coinbase exact, and push at `/submit`, hoping a supply commitment that is internally self-consistent with fabricated uncles slips past before the real, PoW-gated uncle validation ever runs. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#fabricatedUncleRefsThatMakeTheSupplyCommitmentSelfConsistentAreStillRejectedAtRealUncleValidation` |
| E2E-67 | Serve a real syncing node, under a curve-active profile, a headers-only response whose final header carries a forged supply copied from another real height on the same source chain rather than an arbitrary offset, over a real socket, hoping a curve-plausible forgery survives real parsing, caps and deadlines where an arbitrary one would not. | A2 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveHeaderSyncTest.java#aCurvePlausibleForgedSupplyInAHeadersOnlyResponseLeavesTheVictimsChainUntouched` |
| E2E-68 | Mine an honest prefix under a curve-active profile, then serve a header stream whose final header drops an already-committed supply back to the absent sentinel, hoping the dedicated absent-sentinel guard in `HeaderChain.checkSupply` is somehow conditioned on the curve being active rather than applying unconditionally. | A2 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveHeaderSyncTest.java#aHeaderThatDropsAnAlreadyCommittedSupplyMidCurveIsRejectedBeforeAnyBodyIsFetched` |
| E2E-69 | Forge a block at `emissionCurveHeight - 1` claiming the curve formula instead of the still-in-force geometric one, and its mirror at exactly `emissionCurveHeight` claiming the now-stale geometric value instead of the curve, pushed at the real `/submit` route, hoping the activation predicate flips one block early or one block late. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveBoundaryTest.java#theCurveActivatesAtExactlyItsScheduledHeightNeitherOneBlockEarlyNorLateOverTheSubmitRoute` |
| E2E-70 | The same activation boundary over a real headers-only sync instead of `/submit` -- an honest pre-activation prefix, then a final header claiming the stale geometric reward exactly at the activation height, served by a hostile peer over a real socket -- hoping the header-sync ingress path applies the activation predicate differently from the block-submit path. | A2 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveBoundaryTest.java#theCurveActivationBoundaryHoldsOverAHostileHeaderSyncToo` |
| E2E-71 | Restart a real mining node (same RocksDB data directory) exactly at its curve activation height and resume mining across that boundary, hoping the recomputed emission table or the restarted node's own next reward disagrees with a peer that was never restarted -- the same difficulty-recomputation-on-restart bug class `CLAUDE.md` calls out for Pandanite, applied to the emission table. | A6 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveBoundaryTest.java#aRestartExactlyAtTheActivationHeightReloadsTheSameHistoricalRewardsAndContinuesMiningWithTheCurveActive` |
| E2E-72 | Pair a curve-inactive real node with a curve-active real node sharing an identical genesis and chainId -- an invisible-at-the-transport-layer misconfiguration, since `emissionCurveHeight` is not part of genesis identity -- and let the curve-active node mine and gossip to the other, hoping one silently adopts the other's chain instead of forking cleanly. | A0 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveMisconfigurationTest.java#aCurveActivePeerAndACurveInactivePeerWithIdenticalGenesisForkCleanlyAndNeitherAdoptsTheOther` |
| E2E-73 | Fork two real, independently-mining nodes under a curve-active profile with no lies -- a genuine partition, each mining its own branch from real divergent parent supplies -- let them reorg to the heavier branch over real HTTP sync, and see whether the two nodes' independently-read, real committed supplies actually agree once converged. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveEmissionTest.java#twoForkedMiningNodesUnderACurveActiveProfileThatConvergeAgreeOnSupplyAndRewardAtTheSettledHeight` |
| E2E-74 | Force a real fork-then-reorg whose boundary is exactly the block whose parent supply crosses the curve's supply target -- the losing branch staying on the positive/interpolated side, the winning branch crossing into the negative/mirrored side on its own honestly-mined block -- hoping the reorg-losing node ends up disagreeing with the node that mined the crossing branch natively, or that `Executor.rollbackBlock` mishandles a reward that is not a pure function of height alone under the curve. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveReorgTest.java#aReorgThatSwapsBranchesAcrossTheSupplyTargetCrossingReversesAndReappliesRewardsExactly` |
| E2E-75 | Mine a real node, with a genuine live block producer, far past the curve's supply target into the sustained region where the honest reward is pinned to exactly `R_min` for many consecutive real blocks (the floor's permanent tail emission), hoping the sustained floor eventually gets confused with an unrelated mining-fee status, or that the real `/wallet` balance route or some other downstream code silently assumes a different coinbase. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#aRealNodeMinedDeepPastTheSupplyTargetProducesManyFloorRewardBlocksAndStaysHealthy` |
| E2E-76 | Forge a header whose base term exactly equals the curve's raw value at the real parent supply, but whose uncle/nephew contribution uses the pre-curve, height-only reward divisors instead of their curve-sensitive twins, hoping the header-sync gate checks the base term carefully but is laxer about the uncle contribution. | A2 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveHeaderSyncTest.java#aHeaderWhoseUncleContributionUsesTheStaleHeightOnlyDivisorsInsteadOfTheCurveSensitiveOnesIsRejected` |
| E2E-77 | Serve a forged header claiming a parent supply near `Long.MAX_VALUE`, hoping the real sync thread crashes on an uncaught exception rather than cleanly rejecting it. | A2 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveHeaderSyncTest.java#aForgedSupplyNearLongMaxValueIsRejectedWithoutCrashingTheSyncThread` |
| E2E-78 | Stagger `consensusV2Height` and `emissionCurveHeight` in both possible orders, and exactly together, on a real chain, submitting under-floor-fee and stale-reward transactions around each boundary, hoping the two independent height-dispatched gates -- the transaction fee floor and the emission schedule -- ever interact badly when both are reached in the same real block. | A1 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveConsensusV2Test.java#consensusV2ActivatesBeforeTheCurveAndEachGateStillFlipsExactlyAtItsOwnHeight`, `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveConsensusV2Test.java#theCurveActivatesBeforeConsensusV2AndEachGateStillFlipsExactlyAtItsOwnHeight`, `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveConsensusV2Test.java#bothGatesActivateInTheExactSameBlockAndNeitherMasksTheOther` |
| E2E-79 | Bootstrap a fresh curve-active node purely via snap-sync -- so it never replays chain history through `Executor` -- against an honest peer already several blocks past `emissionCurveHeight`, then have the synced node mine its own next block, hoping its coinbase disagrees with what a peer that replayed everything would compute. | A0 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveSyncTest.java#aSnapSyncedCurveActiveNodeMinesACoinbaseMatchingAFullyReplayedPeersAtTheSameHeight` |
| E2E-80 | Repeat a real fork-and-reorg at the `supplyTarget` frontier a dozen-plus times in a row on the same chain, hoping the documented ≤1-base-unit reorg discontinuity compounds across repeated events instead of staying bounded. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveReorgTest.java#repeatedReorgsAcrossTheSupplyTargetFrontierDoNotCompoundTheBoundaryDiscontinuity` |
| E2E-81 | Force a real reorg whose boundary lands exactly on the curve's own activation height, hoping a block ends up paid according to a decision cached at assembly time rather than the curve rule in force at whichever height it actually occupies on the winning branch. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveReorgTest.java#aReorgThatCrossesTheEmissionCurveActivationHeightPaysTheCurveRateAtTheRealPositionNativelyAndAfterReorg` |
| E2E-82 | Repeat the fork-and-converge supply proof under the real, shipped mainnet calibration (`supplyTarget`, `emissionCoefficient` and 256 table steps from `NetworkParameters.cleanMainnet()`) on fast PoW, through a real reorg, hoping the ≤1-unit reorg discontinuity -- proven so far only on a 16-step toy table -- is exceeded on the much finer N=256 interpolation grid. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2ECurveReorgTest.java#theForkAndConvergeSupplyProofHoldsUnderTheRealMainnetCalibratedTableAtItsFullResolution` |
| E2E-83 | Serve a long, honest, real-PoW-valid header run whose committed supply crosses `supplyTarget` partway through, hoping the repeated `BigInteger` evaluation of the curve's mirror branch, once per header before that header's own PoW re-check, measurably stalls sync or leaves the node unresponsive to its other peers or the API. | A3 | DEFENDED | `app-node/src/test/java/rhizome/adversarial/e2e/E2EEmissionCurveTest.java#syncingALongRealCurveCrossingChainStaysWithinPatienceAndTheVictimStaysResponsiveThroughout` |
...
- **2026-08-25** — New `FLOOR` family (FLOOR-01..04) closes the miner revenue floor's adversarial
  ...
  floored regime. The floor replaces feature 03's zero clamp at the curve's single site
  (`max(R_min, raw(S))`, `R_min = 800` base units on mainnet): above the target and through the
  crossover band the honest reward is exactly `R_min`, never zero. `MinerRevenueFloorAttackTest`
  proves the dispatched reward floors at exactly `R_min` across the whole reachable domain
  including wire-legal extremes and that an overflowing accounting identity rejects loudly
  (FLOOR-01), a validator enforces the floored schedule exactly above the target (FLOOR-02),
  uncle/nephew rewards floor through the base with exact supply identity and reversal (FLOOR-03),
  and a non-positive floor refuses to build (FLOOR-04). E2E-64/65/75 (renamed to
  `aRewardOffTheFloorClaimedAboveTheSupplyTargetIsRejectedAndTheNodeStaysHealthy`,
  `theRealNodeKeepsProducingThroughTheFloorCrossoverBandAndRejectsAnyExtraUnitClaimedThere`,
  `aRealNodeMinedDeepPastTheSupplyTargetProducesManyFloorRewardBlocksAndStaysHealthy`) re-prove the
  floored regime through real nodes; E2E-84/85 were re-premised alongside (honest above-target
  reward is exactly `R_min`).
- **2026-08-24** — Twenty-six new `E2E` scenarios (E2E-61..86) close out the integer log emission
  curve's adversarial test plan (branch `004-integer-log-curve`) except the two the plan itself
  carved out: CURVE-09, redundant with E2E-64 and merged into it rather than implemented, and
  CURVE-20, an infra gap the plan deferred. The GraalVM row keeps its reserved E2E-88 id; the
  next new scenario starts at E2E-87. Eight new suites, all real-node, real-socket proofs.
  `E2EEmissionCurveTest` carries the fixture-regression proof that a curve-aware supply stamp
  beside a curve-insensitive coinbase is refused (E2E-61), a neighbouring table position's reward
  at the real `/submit` route (E2E-62), a one-unit-overstated coinbase at an exact generated
  table position — reached by a premined genesis, since honest mining's consensus-chosen reward
  granularity cannot land on a step boundary (E2E-63), the zero-clamp above and the genuine
  zero-reward band below `supplyTarget` (E2E-64/65), a supply commitment self-consistent with
  fabricated uncles (E2E-66), sustained zero-reward production deep past the target (E2E-75),
  and the `BigInteger` mirror-branch cost paid before PoW under a long honest sync, a
  PoW-invalid submit flood, and concurrent competing submits (E2E-83..86).
  `E2ECurveHeaderSyncTest` serves the curve-active twins of the header-sync forgeries
  (E2E-67/68) plus the stale uncle-divisor and near-`Long.MAX` parent-supply headers (E2E-76/77).
  `E2ECurveBoundaryTest` flips the activation predicate exactly at `emissionCurveHeight` over
  both ingress paths (E2E-69/70) and across a real RocksDB restart (E2E-71).
  `E2ECurveMisconfigurationTest` forks a curve-active and a curve-inactive node sharing one
  genesis cleanly apart (E2E-72). `E2ECurveEmissionTest` and `E2ECurveReorgTest` prove forked,
  independently-mining nodes and reorg-swapped branches agree on committed supply and pay the
  curve rate at the position a block actually occupies (E2E-73/74, 80..82) — including under the
  shipped mainnet calibration at full N=256 resolution. `E2ECurveConsensusV2Test` staggers
  `consensusV2Height` and `emissionCurveHeight` in both orders and exactly together (E2E-78).
  `E2ECurveSyncTest` snap-syncs a fresh curve-active node and has it mine a coinbase matching a
  fully-replayed peer's at the same height (E2E-79).
