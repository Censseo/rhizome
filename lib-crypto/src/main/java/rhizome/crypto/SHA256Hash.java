...
public record SHA256Hash(byte[] hash) implements Comparable<SHA256Hash> {
    ...
    public static SHA256Hash empty() {
        return new SHA256Hash(Crypto.emptyBytes(SIZE));
    }

    public static SHA256Hash random() {
        return new SHA256Hash(Crypto.randomBytes(SIZE));
    }
